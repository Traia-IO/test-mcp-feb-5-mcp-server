...
# Script to build and run the Test MCP Feb 5 MCP Server locally in Docker
...
IMAGE_NAME="test-mcp-feb-5-mcp-server"
CONTAINER_NAME="test-mcp-feb-5-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Test MCP Feb 5 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Test MCP Feb 5 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Test MCP Feb 5 MCP Server is running!${NC}"
echo
